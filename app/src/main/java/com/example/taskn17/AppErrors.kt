package com.example.taskn17

import com.example.taskn17.authentfication.ErrorResponse
        ...
        fun fromException(exception: Exception): AppError {
            return when (exception) {
                ...
                is HttpException -> {
                    when (exception.code()) {
                        in 400..499 -> {
                            val adapter: JsonAdapter<ErrorResponse> = Moshi.Builder().add(KotlinJsonAdapterFactory()).build().adapter(ErrorResponse::class.java)
